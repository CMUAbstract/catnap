...
PERIOD(p1, sec_to_tick(0.1), sec_to_tick(0.1));
// Period for running fft. variable

PARAM(sample_size, SAMPLES, SAMPLES);

FUNCS(f_mic, event_mic);
FUNCS(f_start_fft, event_start_fft);
FUNCS(f_tx, event_transmit);
FUNCS(f_fft, task_fft);

EVENT_PERIODIC(e_mic, f_mic, p1, 1);
EVENT_PERIODIC(e_fft, f_start_fft, p1, SAMPLES);
EVENT_APERIODIC(e_tx, f_tx, p1, SAMPLES);
...
void event_start_fft(unsigned param)
{
	P2OUT |= BIT5;

	if (!IS_FIFO_EMPTY(e_mic, t_fft)) {
		PRINTF("FFT ready\r\n");
		POST(t_fft);
		POST_EVENT(e_mic);
	} else {
		PRINTF("FFT not ready\r\n");
	}
